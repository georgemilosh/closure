...
def read_variables(f):
    global rho,p, b, gb, db, gyro_radius, vth, e, ex, ey, ez, bx, by, bz, az, jx, jy, jz, rhoc, rho0, rho1, rho2, rho3, bx_ext, by_ext, bz_ext, ex_c, wp, lambdOdx, va
 ...
 #   p = pxx + pyy + pzz
    ...
    ex_c = compute_ex(Nx,Ny,dx,dy,rhoc)
    
    c = 1
    wc = qom*b/c
    wp = np.sqrt(4*np.pi*abs(rho0*qom))
    lambd = uth / (wp+1e-10)
    lambdOdx = lambd /dx
    
    va = b/np.sqrt(4*np.pi*rho1)
...
def compute_ex(Nx,Ny,dx,dy,rhoc):
    ex_c = np.zeros((Ny,Nx))
    tmp = np.zeros((Ny,Nx))
    for i in range(20,Nx):
        for j in range(0,Ny):
            tmp[j][i] = tmp[j][i-1] + rhoc[j][i]  * dx / 4.0 /np.pi
#np.flip(tmp,axis=1)
#ex_c = np.cumsum(tmp,axis=1)
#np.flip(ex_c,axis=1)
    for i in range(0,Nx):
        for j in range(0,Ny):
            ex_c[j,i] =  tmp[j,i] #/ (i * dx+1e-10) *dx
    return ex_c
...
#  Plot Variable
#==============================================================================
def plot_variables(time_label, Lx, Ly, variable, az, name_variable, minval, maxval,signed):
	global directory
	levels = 20
	#variable_sm=ndimage.filters.gaussian_filter(variable, 2, mode='nearest')
	if minval==0 and maxval==0:
		varin = variable[10:-10,10:-10]
		minval = round_to_n(varin.min(),4)
		maxval = round_to_n(varin.max(),4)
...
#CB = plt.colorbar(CL, extend='both')
	...
	CBI = plt.colorbar(CF, ticks=[mn, md,mx],orientation='vertical')
...
#plt.axes().set_aspect('equal')
	print('plotted', name_variable)
	plt.show()
	fname=directoryOUT+name_variable+time_label+'.png' # .svg also works very well
...
small=1e-50*qom;
for i in range(ini+rank*dt,nt+dt,dt*size):
    print(i)
    time_label = str(i).zfill(6)
    filename = directory+'FORCED-Fields_' + time_label +'.h5'
    #print filename
    ...
    read_variables(f)
    plot_variables(time_label,Lx, Ly, bx, az, 'bx',0,0,0)
    plot_variables(time_label,Lx, Ly, by, az, 'by',0,0,0)
    plot_variables(time_label,Lx, Ly, bz, az, 'bz',0,0,0)
    #plot_variables(time_label,Lx, Ly, bx_ext, az, 'bx_ext',0,0,0)
    #plot_variables(time_label,Lx, Ly, by_ext, az, 'by_ext',0,0,0)
    #plot_variables(time_label,Lx, Ly, bz_ext, az, 'bz_ext',0,0,0)
    
    plot_variables(time_label,Lx, Ly, ex, az, 'ex',0,0,0)
    plot_variables(time_label,Lx, Ly, ey, az, 'ey',0,0,0)
    plot_variables(time_label,Lx, Ly, ez, az, 'ez',0,0,0)
    
    #plot_variables(time_label,Lx, Ly, rhoc_avg, az, 'rhoc_avg',0,0,0)
    #plot_variables(time_label,Lx, Ly, divE_avg, az, 'divE_avg',0,0,0)
    ...
    for ispec in range(0,ns):
        ...
        read_variables(f)
        print(spec)
        plot_variables(time_label,Lx, Ly, uth, az, 'uth'+spec,0,0,1)
        plot_variables(time_label,Lx, Ly, vth, az, 'vth'+spec,0,0,1)
        plot_variables(time_label,Lx, Ly, wth, az, 'wth'+spec,0,0,1)
    
        plot_variables(time_label,Lx, Ly, jx, az, 'jx'+spec,0,0,0)
        plot_variables(time_label,Lx, Ly, jy, az, 'jy'+spec,0,0,0)
        plot_variables(time_label,Lx, Ly, jz, az, 'jz'+spec,0,0,0)
        plot_variables(time_label,Lx, Ly, jy/rho/va, az, 'vyOva'+spec,0,0,0)

            
        plot_variables(time_label,Lx, Ly, rho, az, 'rho'+spec,0,0,qom/np.abs(qom))
#plot_variables(time_label,Lx, Ly, np.log10(abs(lambdOdx)), az, 'lambdOdx'+spec,-2,1,1)
