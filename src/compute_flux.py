...
data, X, Y, qom, times = rp.get_exp_times(experiments, files_path, fields_to_read, 
...
for xi in xs:
    ...
    filtered['Em_bar'].append(np.mean(data[experiment]['Em_bar'], axis=(0,1)))
    for quantity in ['PIuu', 'PIbb', 'Ef_favre', 'PS', '-Ptheta', 'JdotE']:
        for species in ['e', 'i']:
