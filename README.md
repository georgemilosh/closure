...
**Evaluate + plot** — `eval_test_ablations_interactive.sh` (a thin wrapper around
...
- `figs/fig_channel_r2_<model>_<split>_<target>.png` — per-channel R² heatmaps
  (target channel × feature × arch). Generated by the eval itself — no per-folder
  `make_figures.py` needed; set `SKIP_FIGURES=1` to skip.
