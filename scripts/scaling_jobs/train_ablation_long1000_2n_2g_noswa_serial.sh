#!/bin/bash
# Serial 1000-epoch ablation for all 4x2 feature/target combinations (2 nodes, 2 GPUs per node, DDP, no SWA).
# Runs all 8 combinations back-to-back (default, noE, noJ, noJnoE × P, divP).
# Total estimated runtime: ~16 hours (8 × ~1.5-2 hours per combination with inter-node overhead).
# Submit with:
#   sbatch models/Lightning/iPiC3D-nathan5-12/scaling_jobs/train_ablation_long1000_2n_2g_noswa_serial.sh
#SBATCH --job-name=ipic_ablate_long1000_2n_2g_noswa_serial
...
echo "=== Starting serial 1000-epoch ablation matrix (2 nodes, 2 GPUs per node, DDP, no SWA, all 8 combinations) ==="
...
run_ablation() {
  ...
  srun --cpu-bind=none closure-train fit \
    ...
    --trainer.logger.init_args.save_dir=./models/Lightning/iPiC3D-nathan5-12/ablations_long1000_serial_2n_2g_noswa \
    --trainer.logger.init_args.name=runs \
    --trainer.logger.init_args.version="${run_tag}" \
    --trainer.default_root_dir="./models/Lightning/iPiC3D-nathan5-12/ablations_long1000_serial_2n_2g_noswa" \
    $feature_override \
    $target_override \
    $prescaler_override \
